...
extension NKZEndRecord {
    ...
    static func findEndRecordInBytes(bytes: UnsafePointer<UInt8>, length: Int) -> NKZEndRecord? {
        
        var reader = NKZBytesReader(bytes: bytes, index: length - 1 - signature.count)
        ...
        return NKZEndRecord(numEntries: numEntries, centralDirectoryOffset: centralDirectoryOffset)
    }
